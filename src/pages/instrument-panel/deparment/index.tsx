...
export default function index() {
  ...
  const columns: any = [
    ...
    },
    {
      ...
      key: 'do',
      align: 'center',
      render: (item, departData) => (
        <span>
          {
            <Link to={`/instrument-panel/department/editDepartment?id=${departData.key}`} className="noticeLink">
              编辑
            </Link>
          }
        </span>
    ...
    },
    {
      ...
      align: 'center',
      render: (item, departData) => (
        <span>{<Link to={`/instrument-panel/department/configDepartment?id=${departData.key}`}>查看配置</Link>}</span>
      ),
      width: '8%',
      // 设置文本过长用省略号代替并关闭提示的title
      ellipsis: {
  ...
  const departmentReset = () => {
    departmentInput.departName === '' && departmentInput.departUser === '' && departmentInput.departPlace === ''
      ? null
      ...
      <div className="departmentMain">
        ...
        <div className="departmentAddBox">
          <Button
            ...
            onClick={() => {
              navigator('/instrument-panel/department/addDepartment')
            }}
          >
            +&nbsp;&nbsp;新增部门
          </Button>
