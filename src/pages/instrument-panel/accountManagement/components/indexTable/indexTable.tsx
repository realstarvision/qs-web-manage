...
import { findOrderListByPage } from '@/api/workOrder'
...
export default function indexTable() {
  ...
  // 分页总数
  const showTotal = (total) => `${'共有'} ${total} ${'条数据'}`
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, record) => <span>{record.dd}</span>,
    },
    ...
    },
    {
      ...
      render: (_, record) => <span>{record.cc}</span>,
    },
    {
      title: '角色',
      dataIndex: 'role',
    ...
    },
    {
      ...
      render: (_, record) => (
        <Badge
          // 根据状态码判定显示颜色
          status={record.orderState === 0 ? 'success' : 'error'}
          text={record.orderState === 0 ? '正常' : '禁用'}
        />
      ),
    },
      ...
      render: (_, record) => (
        <p>
          <span style={{ marginRight: '8px' }}>
            <Link
              to={`/instrument-panel/accountManagement/editAccountManage?id=${record.key}`}
          ...
          <span>
            <Link
              to={`/instrument-panel/accountManagement/watchAccountManage?id=${record.key}`}
              onClick={(e) => handleCheck(e, record)}
            >
              操作记录
            </Link>
  ...
  // 导出excel
  const downloadExcel = (tableRecord, selectedRowKeys) => {
    //导出table数据
      ...
      <Box className="accountTableOutBox">
        <div className="accountTableButtonBox">
          <p className="accountTableButtonSel">
            已选
            <span className="accountTableButtonSelLength">{selectedRowKeys ? selectedRowKeys.length : 0}</span>条
          </p>
          <Popconfirm
            title="是否禁用该数据"
            okText="确定"
          ...
          <Button
            ...
            onClick={() => {
              downloadExcel(tableRecord, selectedRowKeys)
            }}
          >
            导出
          </Button>
